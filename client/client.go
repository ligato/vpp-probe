...
func RunOnInstances(instances []*vpp.Instance, workFn func(*vpp.Instance) error) error {
	...
	logrus.Debugf("waiting for results")
	...
	logrus.Tracef("all instances finished (took %v)", time.Since(start).Round(time.Second))
	var total time.Duration
	for _, res := range results {
		logrus.Tracef("- %+v", res)
		total += res.Elapsed
	}
	logrus.Tracef("total elapsed time: %v", total.Round(time.Second))
