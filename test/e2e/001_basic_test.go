...
func (s *BasicTestSuite) TestTopoDiscover() {
	t := s.T()

	cli, err := cmd.NewProbeCli()
	s.NoError(err)

	T1, err := kube.NewProvider("", s.kubectx)
	...
	if err := c.DiscoverInstances(cli.Queries()...); err != nil {
		t.Fatal(err)
	}

	for _, instance := range c.Instances() {
		fmt.Sprintf("- checking instance %+v: %v", instance.ID(), instance.Status())
	}
}
